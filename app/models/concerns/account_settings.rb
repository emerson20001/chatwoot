module AccountSettings
  ...
  def normalize_custom_menu_item(item)
...

    {
      ...
      'token_id' => normalize_custom_menu_text(item, :token_id),
      'blog_id' => normalize_custom_menu_text(item, :blog_id),
      'url_blog_id' => normalize_custom_menu_text(item, :url_blog_id),
  ...
  def normalize_custom_menu_text(item, key)
    raw_value = if item.key?(key.to_s)
                  item[key.to_s]
                elsif item.key?(key)
                  item[key]
